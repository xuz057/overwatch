...
```{r echo=FALSE, message=FALSE, warning=FALSE}
AVERAGE_WIN_RATE <- GameRecordS13R%>%group_by(PLAYER_ID)%>%mutate(AVG_WIN=sum(WIN)/sum(GAME_PLAYED))%>% select(PLAYER_ID,AVG_WIN)
